...
export function enhancedNormalize(text) {
...
export function calculateAdvancedSemanticScore(chunk, query, conversationHistory) {
...
// ========= RESPONSE SYNTHESIS =========
export function synthesizeResponse(matches, intent, query) {
...
// ========= CORE LOGIC =========
export function generatePseudoLLMAnswer(question, sentenceChunks, conversationHistory) {
  const intent = parseIntent(question);
  const scores = sentenceChunks.map(chunk => ({
    chunk,
    score: calculateAdvancedSemanticScore(chunk, question, conversationHistory)
...
// ========= Initialize Sentence Chunks =========
export function initializeSentenceChunks() {
  return chatbotProfile.map((sentence, index) => ({
    text: sentence.trim(),
