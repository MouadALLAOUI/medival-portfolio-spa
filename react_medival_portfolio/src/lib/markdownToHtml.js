...
function parseCodeBlocks(markdown) {
  ...
  return markdown.replace(codeBlockRegex, (match, lang, code) => {
    ...
    return `<pre class="code-block-container relative"><code id="${codeId}" class="language-${lang}">${escapedCode}</code><button class="copy-btn" data-target="${codeId}" title="Copy code">📋</button></pre>`;
  });
}
...
function processCurrentBlock(currentBlock, outputLines, toc) {
  ...
  } else if (/^[-*_]{3,}\s*$/.test(blockContent)) {
    outputLines.push("<hr>");
  } else {
